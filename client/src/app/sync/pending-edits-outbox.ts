...
export class PendingEditsOutbox {
   ...
   * the edits worth replaying: the latest write per cell, across all of them. A record is left
   * alone, not harvested, if it was touched recently: that means its tab is still alive (a live
   * tab keeps refreshing its own record; see heartbeat in sheet-sync.service.ts), and taking its
   * edits would race with that tab still holding and eventually sending them itself.
   */
  async harvestStaleOps(sheetId: string, staleAfterMs: number): Promise<CellOp[]> {
    const records = await this.storage.getForSheet(sheetId);
    const cutoff = this.now() - staleAfterMs;
    const stale = records.filter((r) => r.ops.length > 0 && r.lastSeenMs <= cutoff);

    await Promise.all(stale.map((r) => this.storage.delete(r.sheetId, r.nodeId)));

    return mergeLatestPerCell(stale);
  }

  /** Persists (or, once nothing is left, deletes) this tab's own unsent edits for a sheet. */
  async persist(sheetId: string, nodeId: string, ops: readonly CellOp[]): Promise<void> {
    if (ops.length === 0) {
      await this.storage.delete(sheetId, nodeId);
      return;
    }
    await this.storage.put({ sheetId, nodeId, ops: [...ops], lastSeenMs: this.now() });
  }
}
